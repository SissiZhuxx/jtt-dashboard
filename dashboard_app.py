...
mode = query_params.get("mode", ["viewer"])[0] # 默认是访客模式 'viewer'

# --- 数据加载 ---
@st.cache_data(ttl=600) # 缓存10分钟
def load_data_from_repo():
    # 从GitHub仓库直接加载数据 (访客模式)
    try:
        orders_df = pd.read_csv('交团团近3月数据-20250821.xlsx - 订单列表.csv')
        products_df = pd.read_csv('交团团近3月数据-20250821.xlsx - 商品列表.csv')
        return process_data(orders_df, products_df)
    except FileNotFoundError:
        st.error("数据文件在仓库中未找到！请管理员确认 '交团团近3月数据-20250821.xlsx - 订单列表.csv' 和 '商品列表.csv' 已上传至GitHub仓库。")
        return None, None

@st.cache_data # 为上传的文件创建缓存
def load_data_from_upload(orders_file, products_file):
    # 从上传的文件加载数据 (管理员模式)
    orders_df = pd.read_csv(orders_file)
    products_df = pd.read_csv(products_file)
    return process_data(orders_df, products_df)

def process_data(orders_df, products_df):
...
orders_df, products_df = None, None

if mode == 'admin':
    st.sidebar.header('管理员模式：上传新数据预览')
    uploaded_orders_file = st.sidebar.file_uploader("上传订单列表CSV", type="csv")
    uploaded_products_file = st.sidebar.file_uploader("上传商品列表CSV", type="csv")
    if uploaded_orders_file and uploaded_products_file:
        orders_df, products_df = load_data_from_upload(uploaded_orders_file, uploaded_products_file)
        st.sidebar.success("文件上传成功，看板已更新为预览模式。")
    else:
        st.sidebar.info("请上传两个CSV文件以生成预览看板。")
else:
    st.sidebar.header('访客模式')
    st.sidebar.info("您正在查看官方数据看板。")
    orders_df, products_df = load_data_from_repo()
...
if orders_df is not None and products_df is not None:
    ...
    # --- (后续所有分析代码和之前一样，这里为了简洁省略，请直接复制粘贴完整的代码) ---
    # ... (将上一轮回复中 "看板主逻辑" 开始到结尾的所有代码粘贴到这里) ...
    # 为了防止遗漏，下面重新提供完整的看板逻辑代码
    ...
    gmv = unique_b2c_orders['订单金额'].sum(); total_orders = unique_b2c_orders['订单号'].nunique()
    total_customers = unique_b2c_orders['下单人'].nunique(); aov = gmv / total_orders if total_orders > 0 else 0
    ...
    if not b2c_products.empty:
        ...
        with col_cat2:
            if not category_sales.empty:
                ...
                fig2, ax2 = plt.subplots(figsize=(10, 6)); plot_data.plot(kind='pie', autopct='%1.1f%%', startangle=90, ax=ax2)
    ...
    if not unique_b2c_orders.empty and unique_b2c_orders['下单人'].nunique() > 10:
        ...
        try:
            ...
            st.warning(f"RFM分析失败，数据量过少或分布不均。错误：{e}")
